...
internal class SubsetsII : Solution<int[], IList<IList<int>>>
{
    ...
    protected override IEnumerable<(int[], IList<IList<int>>)> TestCases
    {
        get
        {
            yield return (@int[1, 2, 2], List2D("[1,2,2],[],[2],[2,2],[1],[1,2]"));
            yield return (@int[-1, 1, 2], List2D("[[],[-1],[-1,1],[-1,1,2],[-1,2],[1],[1,2],[2]]"));
            yield return (@int[0], List2D("[0],[]"));
            yield return (@int[3, 4, 3, 4], List2D("[[], [4], [3], [3, 4], [4, 4], [4, 3, 4], [3, 3], [3, 3, 4], [3, 4, 3, 4]]"));
        }
    }
